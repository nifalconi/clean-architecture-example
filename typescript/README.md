...
Endpoints are pure factories — they never instantiate a concrete repository.
All wiring happens in one place, `composition-root.ts`, so the rest of the app
...
### Persistence mapping
The data layer keeps its own document shape (`TaskDocument`) and maps it to the
domain `Task` entity, so Mongo concerns (`_id`, timestamps) never leak upward:
```typescript
// data/mappers/task.mapper.ts
export const toEntity = (doc: TaskDocument): Task => ({
  id: doc.id, name: doc.name, date: doc.date,
});
```
...
│   │   ├── database.ts
│   │   ├── models/
│   │   │   └── task.model.ts          # Mongoose schema + TaskDocument type
│   │   ├── mappers/
│   │   │   └── task.mapper.ts         # TaskDocument → Task entity
│   │   └── repositories/
│   │       └── task.repository.ts
│   └── presentation/
│       └── api/
│           ├── get-task.endpoint.ts   # pure factory (no concrete deps)
│           ├── composition-root.ts    # where dependencies get wired
│           └── server.ts              # thin: routing + Bun.serve
├── package.json
├── tsconfig.json
├── bun.lock
└── README.md
